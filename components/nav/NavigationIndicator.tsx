...
export function NavProgressProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <NavProgressContext.Provider value={{ startProgress }}>
      ...
      {showBar && (
        <div
          className="pointer-events-none fixed left-0 top-0 z-[99] h-[4.5px] w-full bg-[#2555F3]/25 md:h-1 md:bg-[#2555F3]/15"
          aria-hidden
        >
          <div className="h-full w-[35%] animate-nav-progress bg-[#2555F3]" />
        </div>
      )}
